...
weavra --help     # Weavra 제품 도움말
weavra --version  # Weavra development (runtime 0.85.1); 내부 버전과 제품 identity 구분
```
...
- 일반 실행은 준비된 Weavra agent dir를 요구한다. 없으면 `Run: weavra setup`으로 종료하며 worktree mutation보다 먼저 검사한다. inherited experimental onboarding은 제거했다. auth가 없더라도 `/login`을 위해 실행 가능하며 손상 JSON/unsafe auth 권한은 거부한다.
- child 직전에 `WEAVRA_CODING_AGENT_DIR=<canonical Weavra agent dir>`로 설정하고 inherited `PI_CODING_AGENT_DIR`/`PI_CODING_AGENT_SESSION_DIR`을 제거한다. `--session-dir` > `WEAVRA_CODING_AGENT_SESSION_DIR` > settings.sessionDir > default agent/sessions/cwd 순서를 유지한다. home은 worktree별로 만들지 않는다.
...
직접 local CLI를 실행해도 기본 user state는 `~/.weavra/agent`다. 다만 launcher의 private permission/overlap 검사와 명시적 setup을 우회하므로 기본 사용은 `weavra`를 권장한다. 명시적 settings/session override·프로젝트 `.pi`·Provider 환경변수/실행 코드 전체를 sandbox하지 않는다. 기존 Pi 파일·global pi/PATH/shell rc를 수정하지 않고 Runtime `.ai` authority와 의미도 그대로 유지한다.
