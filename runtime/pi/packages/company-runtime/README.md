...
| `workflow.prepare` | goal·선택적 recipe data·AC prose로 Runtime preview 준비; Provider 호출 없음. 같은 호스트에서 죽었음이 증명된 소유자의 Run을 INTERRUPTED로 정리한 요청은 준비하지 않고 `STALE_PROJECT`로 답한다([S2](#저장소와-소유권)) |
...
- 브라우저 disconnect·Project panel 이탈은 **server-owned 실행을 종료하지 않는다.** reconnect는 fresh canonical state를 읽고 mutation을 자동 재전송하지 않는다. 이전 UI snapshot·receipt로 approval/실행을 복원하지 않는다. backend/Runtime 종료 뒤 resume을 제공한다는 뜻도 아니다. 죽은 소유자가 남긴 Run은 다음 `workflow.prepare`가 INTERRUPTED로 정리하고(`STALE_PROJECT` 응답) 재개하지 않는다.
...
- **Planning Context.** 첫 user message는 `role: "Planner"` JSON 하나다. goal, prepare 순서의 `AC-001…`, 실행 모드·Risk, 고정 plan 규칙, 등록 check의 id·kind·required·`exercises`, allowed paths, `runtime_list_files`와 같은 규칙의 파일 이름 목록(최대 500개·64 KiB·깊이 4, `.ai/`·보호 경로 제외), 설정된 project instruction snapshot, VALID fact를 담는다. 파일 내용·check 명령·`.ai` 상태·Run 이력·secret은 없다. 196,608 bytes를 넘거나 instruction을 bound 안에서 읽지 못하면 모델 호출 전에 `CONTEXT_TOO_LARGE`이고 잘라내지 않는다.
- **check exercises(개정 A1, #65).** 각 check의 `exercises`는 그 check의 verifier source가 import하는 allowed paths 안의 파일이다(`src/check-exercises.ts`). 직접 verifier source와 선언된 trust 파일(browser 구현 파일 제외, check당 최대 16개)을 trust snapshot과 같은 경로 규칙과 bounded strict UTF-8 reader로 262,144 bytes까지 읽고, `import … from`·`import "…"`·`export … from`·`import("…")`·`require("…")`에서 `./`·`../`로 시작하는 escape 없는 문자열 literal만 찾는다. 실행·평가는 없다. source 디렉터리 기준으로 정규화한 뒤 allowed paths 안이고 listing Policy(`isListablePath`)를 통과하며 verifier source가 아닌 경로만 남긴다. 확장자가 있으면 적힌 대로(없는 파일은 `create` 대상), 없으면 8개 JS/TS 확장자와 `index.*` 후보 중 정확히 하나일 때만 쓴다. 정렬 뒤 check당 16개·context당 64개로 자르고, 잘린 check에만 `exercisesTruncated: true`를 붙인다. 안전하게 읽을 수 없는 source는 건너뛰고 계획은 계속된다. verifier 파일 이름·내용·명령·인자는 context에 없다. plan 규칙 한 줄이 claim을 고를 때 exercises를 우선하게 하지만 권한은 없다.
...
- `FileStateStore.recoverDeadOwner(projectPath, options?)`: Host Control `workflow.prepare` 전용이다. `.ai`를 만들거나 빈 lock을 잡지 않는다. 위 조건과 recovery guard로 소유자가 죽었음이 증명된 lock만 지우고 다음 writer로 열어, 아래의 기존 복구를 그대로 적용한다. COMPLEX는 끝나지 않은 행이 INTERRUPTED/OWNER_LOST가 되고 COMPLETED 행은 유지된다. 그 뒤 lock을 놓고 죽은 PID를 돌려준다. 증명할 수 없으면 아무것도 쓰지 않고 `undefined`다. Host는 client가 본 revision에서만 복구를 시도하고, preview는 항상 요청의 `expectedProjectRevision`에 묶는다. Run을 정리하면 project revision이 바뀌므로 그 prepare는 준비 없이 `STALE_PROJECT`로 답한다. client가 새로 읽은 뒤 보낸 다음 prepare가 복구된 revision에서 정상 준비된다. 정리할 Run 없이 죽은 lock만 지운 경우 revision이 그대로라 같은 prepare가 이어진다. `control.snapshot`은 복구하지 않으며 `workflow.confirm`의 guarded start는 계속 `recoverInterrupted: false`다.
