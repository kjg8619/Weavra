...
When an interactive session starts in a project with resources that require trust and no saved decision for the current directory or a parent directory, pi follows `defaultProjectTrust` from global settings. The default value is `"ask"`, which asks whether to trust the project when UI is available. Saved decisions are stored by canonical directory in `~/.weavra/agent/trust.json`, and the closest saved decision on the current or parent path applies before the global default.
...
- run the whole `weavra` process inside a container/sandbox
- run host `weavra` while routing built-in tool execution into a Gondolin micro-VM
- mount only the workspace paths the agent should access
- avoid mounting host `~/.weavra/agent` unless the container should access host sessions, settings, and credentials
...
For security reporting, follow the [root Weavra repository guidance](../../../../../README.md). Do not open a public issue for security-sensitive reports; inherited upstream maintainer policies do not promise a Weavra response.
