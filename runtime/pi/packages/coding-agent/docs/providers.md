...
Pi supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with pi; configured providers may refresh newer catalogs and cache them in `~/.weavra/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.weavra/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/kjg8619/Weavra/blob/main/runtime/pi/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/kjg8619/Weavra/blob/main/runtime/pi/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.weavra/agent/auth.json`:
...
```bash
weavra --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
weavra --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
weavra --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
weavra --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```
