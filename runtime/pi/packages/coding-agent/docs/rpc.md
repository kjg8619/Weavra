...
The current session name is available via `get_state` in the `sessionName` field. To set the initial name when starting RPC mode, pass `--name <name>` or `-n <name>` to the `weavra --mode rpc` process.
...
const agent = spawn("weavra", ["--mode", "rpc", "--no-session"]);
