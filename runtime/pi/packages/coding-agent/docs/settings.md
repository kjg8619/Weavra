...
| Location | Scope |
|----------|-------|
| `~/.weavra/agent/settings.json` | Global (all projects) |
...
| `enableInstallTelemetry` | boolean | `false` | Unavailable; imported values cannot enable vendor install telemetry or attribution |
...
Weavra does not send vendor install telemetry or consult an upstream product release feed. Imported telemetry settings and legacy flags cannot enable these unavailable services. Explicit provider/package operations have their own network behavior.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.weavra/agent/npm/`; project-scoped npm packages install under `.pi/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
When multiple sources specify a session directory, precedence is `--session-dir`, `WEAVRA_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.weavra/agent/settings.json` resolve relative to `~/.weavra/agent`. Paths in `.pi/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
...
```json
// ~/.weavra/agent/settings.json (global)
{
  "theme": "dark",
