...
Without -l, starts in global settings (<WEAVRA_HOME or ~/.weavra>/agent/settings.json).
...
Update explicitly installed extension packages or configured model catalogs.
Weavra development has no self-updater. Update and rebuild the source checkout that provides this installation.

Options:
  --extensions            Update installed packages only
  --models                Refresh configured model catalogs only
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf =
				source === "self" || source === "pi" || source === APP_NAME || source === "all" || source === "managed";
			if (sourceIsSelf) {
...
/** Keep in sync with the canonical launcher's no-home self-update routing. */
export function isSelfUpdateRequest(args: string[]): boolean {
	return (
		args[0] === "update" &&
		(args.length === 1 ||
			["self", "pi", APP_NAME, "all", "managed"].includes(args[1]) ||
			args.includes("--self") ||
			args.includes("--all"))
	);
}
...
export async function handlePackageCommand(
	...
	if (
		...
		(options.updateTarget?.type === "self" || options.updateTarget?.type === "all")
	) {
		console.error(
			"Weavra development does not support self-update. Update and rebuild the source checkout that provides this installation.",
		);
		console.error(`Use "${APP_NAME} update --extensions" to update explicitly installed extension packages.`);
		process.exitCode = 1;
		return true;
	}
		...
		switch (options.command) {
			...
			case "update": {
				...
				if (target?.type !== "extensions") {
					throw new Error("An explicit extension update target is required.");
				}
