...
export function getPackageDir(): string {
	...
	const envDir = process.env.WEAVRA_PACKAGE_DIR;
	if (envDir) {
...
export const APP_NAME = "weavra";
export const APP_TITLE = "Weavra";
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".pi";
export const VERSION: string = pkg.version || "0.0.0";
/** Development identity, not a release of the internal runtime package. */
export const PRODUCT_VERSION = `${APP_TITLE} development (runtime ${VERSION})`;

// Canonical product-owned environment; inherited Pi homes are never a fallback.
...
// =============================================================================
// User Config Paths (~/.weavra/agent/*)
// =============================================================================

/** Get the Weavra agent directory; project-local .pi resources remain compatible. */
export function getAgentDir(): string {
	const envDir = process.env[ENV_AGENT_DIR];
	if (envDir) {
		return expandTildePath(envDir);
	}
	const home = process.env.WEAVRA_HOME;
	return join(home ? expandTildePath(home) : join(homedir(), ".weavra"), "agent");
}
