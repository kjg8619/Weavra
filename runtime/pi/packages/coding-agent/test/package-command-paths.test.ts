...
describe("package commands", () => {
	...
	it.each([
		...
		["update", "--extension", "--self"],
	])("rejects product update route %j before settings, network, or installation", async (...args) => {
		vi.stubEnv("PI_TELEMETRY", "1");
		vi.stubEnv("PI_SKIP_VERSION_CHECK", "0");
		vi.stubEnv("PI_MANAGED_INSTALL_ROOT", join(tempDir, "legacy-managed"));
		vi.stubEnv("PI_INSTALLER_API_BASE", "https://example.invalid/installer");
		const fetchMock = vi.fn(() => {
			throw new Error("Network forbidden");
		});
		vi.stubGlobal("fetch", fetchMock);
		const spawn = vi.spyOn(childProcesses, "spawnProcess").mockImplementation(() => {
			throw new Error("Install forbidden");
		});
		const spawnSync = vi.spyOn(childProcesses, "spawnProcessSync").mockImplementation(() => {
			throw new Error("Install forbidden");
		});
		const settings = vi.spyOn(SettingsManager, "create").mockImplementation(() => {
			throw new Error("Home access forbidden");
		});
		...
		expect(existsSync(join(tempDir, "legacy-managed"))).toBe(false);
	});
