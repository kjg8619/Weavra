...
## 14. Append-only #28 terminal-delivery CI follow-up

The local results in §13 belong to implementation `74430145d27333dded4c7d6756cca27c84cd6e59` / documentation HEAD `41267be2c5fa8ac2a3c65811cff748d3da9fcd97`. Subsequent remote workflow `35688980409` passed App and cross-boundary but failed Runtime's isolated rerun: the existing experimental-client prompt-event test missed `run_end`. That failure supersedes any merge/closure-ready interpretation of the earlier local result; #15 remains BLOCKED until the repaired HEAD passes all three remote jobs.

The defect is outside Capability Broker semantics. `runClient()` awaited the AgentController RPC response, then unsubscribed and drained only the callbacks already chained into `deliveryTail`. Transcript updates use an independent asynchronous delivery queue. A completed command therefore did not prove that its terminal event had reached the consumer.

The minimal production change is confined to `runtime/pi/packages/coding-agent/src/experimental/client.ts`: remember observed run-end IDs, wait for the accepted response's exact operation ID before unsubscribing, then drain the existing asynchronous callback chain. Hydration is still not replayed. Rejected requests do not wait for a nonexistent terminal event; attachment loss while waiting rejects rather than hanging, and callback failures still propagate. No provider, transport, protocol, Policy, Approval, worker-tool or Kernel contract is changed. The public experimental-client event semantics are not widened: this guarantees the terminal delivery already required by the original test.

The existing real Unix/server/worker integration regression now deterministically controls the consumer-side terminal callback relative to the real RPC response. With terminal delivery deferred to the next event-loop turn, the unmodified client failed with missing `run_end` (1.81s); this is a scheduling barrier, not a sleep/retry-until-green. The retained regression covers late terminal, early terminal, attachment loss, asynchronous callback failure and rejected prompt. Models remain faux. An intermediate isolated run passed all 31 tests; final build/typecheck/full-gate results are recorded in the root work log.

Validation caught two implementation/test-setup mistakes before delivery: Vitest `test.each` does not supply the per-test cleanup context as an extra callback argument, and the root TypeScript library target does not expose `Promise.withResolvers`. Normal context-bearing `test` registrations and the repository's existing Promise-constructor pattern correct these without changing the compiler target, timeout, assertions or skip policy. The package changelog records the terminal-delivery fix. Historical §§1–13 remain unchanged; final repaired HEAD and remote gate disposition are recorded on PR #28.

Final local verification after the compatibility correction: isolated remote-runtime regression **31/31 PASS (Vitest 21.72s)**, full Runtime build PASS, and `node scripts/validate.mjs pi` **ALL GATES PASS**. The combined three-command wall time is 535.69s, not the validator-only duration. Both complete test passes report coding-agent 2,754 tests and company-runtime 1,522 tests passing. Existing skips remain; final check/check:ci apply no source changes. Only the intended client, regression, package changelog and two root evidence documents changed. Remote Runtime/App/cross-boundary results must be checked on the final pushed HEAD independently; local PASS alone does not lift the remote blocker.
