...
| `docs/architecture` | Contracts: [BOUNDARIES](BOUNDARIES.md), [STATE_STORE](STATE_STORE.md), [CAPABILITY_BROKER](CAPABILITY_BROKER.md), [PROJECT_FACTS](PROJECT_FACTS.md), [COMPLEX design](COMPLEX_SEQUENTIAL_WORKFLOW.md), [COMPLEX re-run](COMPLEX_RERUN.md), [PRODUCT_INDEPENDENCE](PRODUCT_INDEPENDENCE.md) | none |
...
COMPLEX ([design](COMPLEX_SEQUENTIAL_WORKFLOW.md)) runs one frozen parent Task Contract as 2–8 human-planned tasks, strictly in order under one Run and one writer. It is prepared only through Host Control with a structured plan (the App's task-plan editor); `/workflow run` refuses it. An optional Planner model can propose a draft for that editor ([PLANNER_DRAFT](PLANNER_DRAFT.md)): a tool-less session whose draft is unreviewed candidate data, validated by the same compiler; the human still loads, edits, prepares and confirms it, and no Planner acts during a Run. `complex-plan.ts` compiles the plan deterministically. Each task runs implement → self-check → contribution review → test. It may change only its exact claimed files (`complex-ownership.ts` checks before Policy and again at the effect). Then integration runs every registered check, a new independent final Reviewer and the checks again. `assertCanCompleteComplex` (`kernel.ts`) decides completion. The App shows the Runtime's `complexExecution` projection and cannot advance tasks. A Run that ends BLOCKED, CANCELLED, FAILED or INTERRUPTED is never resumed. The read-only `workflow.derive` command ([COMPLEX_RERUN](COMPLEX_RERUN.md), `complex-rerun.ts`) can turn the latest such Run into a candidate draft for an ordinary new Run: COMPLETED tasks become read-only verification tasks that are checked again, and the command lists the leftover changes that block a clean start. This is not a resume: the human resolves the leftovers, then prepares and confirms the draft, and the new Run has new identities and fresh evidence for every task.
