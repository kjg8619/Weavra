...
| Whether the interactive `weavra` conversation runs a destructive or unclassified shell command (#5) | the user, per call, in the command guard's prompt; refused without a UI | the local classifier, which only explains; workers have no shell at all |
