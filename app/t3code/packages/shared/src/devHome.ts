 ...
 * `~/.weavra/app` that a user's installed Weavra runs against.
 *
 * A linked git worktree gets its own `.weavra/app`: feature work in a
 * throwaway branch must not share a database with the real app. Canonical
 * `WEAVRA_APP_HOME` and explicit compatibility `T3CODE_HOME` configurations
 * select an explicit base directory instead.
 */
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".weavra", "app");
  });
