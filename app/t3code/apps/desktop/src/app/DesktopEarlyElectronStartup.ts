...
export const resolveLinuxDesktopEntryName = (isDevelopment: boolean): string =>
  isDevelopment ? "io.weavra.desktop.dev.desktop" : "io.weavra.desktop.desktop";
...
function resolveEarlyDesktopSettingsPath(input: {
  ...
  const t3Home = Option.fromNullishOr(
    trimNonEmpty(input.env.WEAVRA_APP_HOME) ?? trimNonEmpty(input.env.T3CODE_HOME),
  );
  const baseDir = resolveDesktopBaseDir({
    homeDirectory: input.homeDirectory,
    joinPath: input.joinPath,
    t3Home,
    weavraHome: Option.fromUndefinedOr(input.env.WEAVRA_HOME),
  });
