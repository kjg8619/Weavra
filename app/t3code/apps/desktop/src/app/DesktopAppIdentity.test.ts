      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: () =>
              input.legacyPathExists
                ? Effect.die("profile resolution must not probe old profiles")
...
describe("DesktopAppIdentity", () => {
  it.effect("isolates userData without probing existing legacy profiles", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/weavra");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Weavra"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Weavra");
        assert.equal(calls.setAboutPanelOptions[0]?.applicationVersion, "development");
