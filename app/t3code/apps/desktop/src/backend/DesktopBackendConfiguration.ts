    ...
    if (runtime.ok) {
      ...
      if (stagedProbe.ok) {
        ...
        return {
          ...
          runtime: { kind: "executable", entryPath: `${runtime.linuxAppRoot}/weavra-server` },
...
> {
  ...
  // Strip Windows-side app/root home overrides so the WSL backend uses its
  // own Linux home instead of sharing the primary's database and credentials.
  const parentEnvWithoutT3Home: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "T3CODE_HOME" || key === "WEAVRA_APP_HOME" || key === "WEAVRA_HOME") continue;
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus app/root home overrides; pass it
