...
<header>↑ Drag Weavra into the list above</header>
<button id="app" draggable="true" aria-label="Drag Weavra to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Weavra</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Weavra icon is missing.");
