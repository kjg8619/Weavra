...
describe("electron development launcher", () => {
  it("uses captured values only as fallbacks for a live runner environment", () => {
    const environmentScript = makeDevelopmentEnvironmentScript({
      ...
      T3CODE_PORT: "16566",
      WEAVRA_APP_HOME: "/tmp/weavra/app",
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths("/repo/apps/desktop/.electron-runtime/Weavra (Dev).app");

    assert.equal(paths.launcherExecutableName, "weavra-desktop");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Weavra (Dev).app/Contents/MacOS/weavra-desktop",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Weavra (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/Weavra (Dev).app/Contents/MacOS/Electron'",
    );
