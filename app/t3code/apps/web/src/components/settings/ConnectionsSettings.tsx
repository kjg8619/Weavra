...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Remote access to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Remote access access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Remote access linked"
          : publishAgentActivity
            ? "Remote access tunnel disabled"
            : "Remote access unlinked",
        description: enabled
          ? "This environment is available through Remote access."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Remote access.",
      });
    }
    setIsUpdating(false);
  };
    ...
    <>
      {window.desktopBridge ? (
        <SettingsRow
          ...
          description={
            managedTunnelActive
              ? "This environment is available to your other devices through Remote access."
              : "Make this environment available to your other devices through Remote access."
          }
      ...
      <SettingsRow
        ...
        description="Send activity to mobile notifications and Live Activities without Remote access."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Remote access."
