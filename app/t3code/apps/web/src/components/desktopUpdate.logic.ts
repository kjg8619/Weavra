...
const DESKTOP_RELEASE_HISTORY_URL = "https://github.com/kjg8619/Weavra";
...
export function getDesktopUpdateReleaseUrl(_version: string | null): string | null {
  return null;
}
...
export function resolveDesktopUpdateButtonAction(
  ...
  if (!APP_UPDATES_ENABLED) return "none";
  if (
...
export function isDesktopUpdateButtonDisabled(state: DesktopUpdateState | null): boolean {
  return !APP_UPDATES_ENABLED || state?.status === "downloading";
}
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  if (action === "download") {
    return "This Mac has Apple Silicon, but Weavra is still running the Intel build under Rosetta. Download the available update to switch to the native Apple Silicon build.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but Weavra is still running the Intel build under Rosetta. Restart to install the downloaded Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but Weavra is running the Intel build under Rosetta. Build the native Apple Silicon app from the Weavra source repository; automatic updates are unavailable.";
}
...
export function getDesktopUpdateInstallConfirmationMessage(
  ...
  return `Install update${version ? ` ${version}` : ""} and restart Weavra?\n\nAny running tasks will be interrupted. Make sure you're ready before continuing.`;
}
