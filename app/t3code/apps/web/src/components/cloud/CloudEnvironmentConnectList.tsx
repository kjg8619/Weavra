  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Remote access.`,
      });
      return true;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Remote access environment.";
      ...
      return (
        <div className={ITEM_ROW_CLASSNAME}>
          <p className="text-sm font-medium text-destructive">
            Could not load Remote access environments
          </p>
  ...
  return visibleEnvironments.map(({ environment, availability, error, status }) => {
    ...
    const statusText =
      unsupported && !savedEnvironment
        ? "Remote access · Not added · Client not supported"
        : savedConnection
          ? savedConnection.statusText
          : availability === "online"
            ? "Remote access · Not added · Relay online"
            : availability === "offline"
              ? "Remote access · Not added · Relay offline"
              : availability === "checking"
                ? "Remote access · Not added · Checking relay status…"
                : (Option.getOrNull(error)?.message ??
                  "Remote access · Not added · Relay status unavailable");
    if (selection) {
      return (
        <label
