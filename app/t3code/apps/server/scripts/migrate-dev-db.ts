 ...
 * ~/.weavra/app database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.weavra/app/userdata/state.sqlite`.
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedError<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .weavra/app directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedError<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.weavra/app database. Use an isolated --base-dir.";
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .weavra/app directory. Defaults to `<worktree>/.weavra/app` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.weavra/app/userdata/state.sqlite`. */
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.String("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .weavra/app directory. Defaults to the current worktree's .weavra/app.",
      ),
    ),
    source: Flag.String("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.weavra/app/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.weavra/app data, then run migrations.",
  ),
);
