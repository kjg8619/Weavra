...
// Persisted metadata cannot override the bundled manifest, even with a future date.
...
describe("ModelManifest service", () => {
  it.live("ignores a newer persisted manifest and never requests remote metadata", () => {
    let requests = 0;
    return Effect.gen(function* () {
      ...
      const cache = encodeJson({
        fetchedAtMs: Number.MAX_SAFE_INTEGER,
        manifest: { ...REMOTE_MANIFEST, updatedAt: "2999-01-01T00:00:00Z" },
      });
      yield* fs.writeFileString(cachePath, cache);
      ...
      assert.deepStrictEqual(yield* restarted.current, BUNDLED_MODEL_MANIFEST);
      assert.strictEqual(yield* fs.readFileString(cachePath), cache);
      assert.strictEqual(requests, 0);
    }).pipe(
      Effect.scoped,
      Effect.provide(
        serviceLayers({
          prefix: "weavra-bundled-model-manifest-",
