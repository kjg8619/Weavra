...
import { updateCommand, resolveLauncherPath } from "./update.ts";
...
it.effect("refuses automatic updates without acquiring network services", () =>
  Effect.gen(function* () {
    const run = Command.runWith(updateCommand, { version: "development" });
    const error = yield* run([]).pipe(Effect.flip);
    assert.strictEqual(error._tag, "CliUpdateError");
