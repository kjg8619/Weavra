...
/**
 * What `weavra-server uninstall` would remove for one Weavra app home. Computed before anything
...
export interface UninstallPlan {
  ...
  /** The `weavra-server` launcher that points into this home's runtime tree. */
  readonly launcher: string | undefined;
  /** `<home>/runtime`, holding explicitly provisioned versions, when it exists. */
...
export const uninstallCommand = Command.make("uninstall", {
  ...
  Command.withDescription(
    "Remove the Weavra background service, owned launcher, and locally provisioned runtimes. Your projects and threads are kept.",
  ),
...
const runUninstall = Effect.fn("cli.uninstall.run")(function* (input: {
  ...
  if (!plan.service && plan.launcher === undefined && plan.runtimeDir === undefined) {
    yield* Console.log(
      `Nothing to remove: Weavra has no managed installation for ${input.baseDir}.`,
    );
    if (!(yield* HostProcessIsExecutable)) {
      yield* Console.log(
        "  This Weavra server runs from a source-built Node script. Remove the explicit launcher or source installation you created; no upstream package manager operation was performed.",
      );
    }
    return;
  }
  ...
  if (plan.runtimeDir !== undefined) {
    yield* Console.log(`  every locally provisioned runtime under ${plan.runtimeDir}`);
  }
  yield* Console.log(
  ...
  if (!input.assumeYes) {
    ...
    const confirmed = yield* Prompt.run(
      Prompt.Confirm({ message: "Remove this Weavra installation?", initial: false }),
  ...
  if (plan.runtimeDir !== undefined) {
    ...
    if (platform === "win32") {
      ...
      yield* Console.log(`${runtimeDir} will be removed once weavra-server exits.`);
    } else {
      yield* fs
  ...
  yield* Console.log("");
  yield* Console.log("weavra-server is uninstalled. Weavra app data was retained.");
});
