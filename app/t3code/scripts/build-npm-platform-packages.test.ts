...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub server that echoes its args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  ...
  for (const key of KEYS) {
    const stem = `weavra-server-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "weavra-server"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "weavra-server"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@t3code/weavra-server-darwin-arm64", "@t3code/weavra-server-linux-x64", "weavra-server"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@t3code/weavra-server-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@t3code/weavra-server-linux-x64");
      assert.equal(linuxManifest.version, VERSION);
      assert.equal(linuxManifest.private, true);
      ...
      // The local tarball must carry node_modules under npm's `package/` root,
      // with the executable bit intact.
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@t3code/weavra-server-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/weavra-server$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(
          process.execPath,
          ["bin/weavra-server.js", "serve", "--port", "1234"],
          {
            cwd: launcherDir,
            env,
          },
        ...
        // the local tarball, including their preflight arguments.
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "weavra-server.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/weavra-server.js", "--version"], {
        cwd: launcherDir,
