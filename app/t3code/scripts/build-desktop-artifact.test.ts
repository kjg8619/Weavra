...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Weavra");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Weavra (Nightly)");
  });
  ...
  it.effect("refuses inherited publish authority even when environment variables name it", () =>
    Effect.gen(function* () {
  ...
  it.effect("omits update feeds for stable and preview artifacts", () =>
    Effect.gen(function* () {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Weavra", schemes: ["t3code", "t3code-dev"] },
      ]);
