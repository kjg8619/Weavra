 ...
 * archive: the `weavra-server` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   weavra-server | weavra-server.exe   the single-executable
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `weavra-server-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
/** Signs weavra-server.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning weavra-server.exe",
  );
  yield* Effect.log("[cli-archive] Signed weavra-server.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "weavra-server.exe" : "weavra-server";
  // tsdown suffixes cross-built executables with their target (weavra-server-darwin-x64);
  // a host build is plain weavra-server. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `weavra-server-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
...
const command = Command.make(
  ...
  Command.withDescription(
    "Package the weavra-server single-executable into a local per-platform archive.",
  ),
);
